...
void Launcher::Draw() {
        ...
        DrawText("Target FPS", 10, 70, 20, BLACK);
        DrawText(std::to_string(fps).c_str(), 200, 70, 20, BLACK);
...
void Launcher::HandleInput() {
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        ...
        if (CheckCollisionPointRec(GetMousePosition(), {10, 40, 280, 20})) {
            AIBlack = !AIBlack;
            hasSettingsChanged = true;
        }
        if (CheckCollisionPointRec(GetMousePosition(), {10, 70, 280, 20})) {
            fps = (fps == 60) ? 120 : 60;
            hasSettingsChanged = true;
        }
    }
